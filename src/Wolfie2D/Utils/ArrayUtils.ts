...
export default class ArrayUtils {
    ...
    /**
     * Returns the index of where elements should be added or deleted from a sorted array.
     * @param arr The array to search through
     * @param element The element to find
     * @returns the index of the element, or where it should be inserted into, or -1 
     * if invalid comaprision.
     */
static binarySearch<T>( arr: Array<T>,element:T,comparator: (arg0: T, arg1: T) => number): number{
        let start=0;
        ...
        while (start<=end){
            ...
            if (arr[mid]==element) 
                return mid;
            else if (comparator(arr[mid], element) < 0)
                 start = mid + 1;
            else
                 end = mid - 1;
        }
